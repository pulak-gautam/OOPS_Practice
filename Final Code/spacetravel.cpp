...
spaceTravel::spaceTravel(Passenger p, int travelDate, int sourceX, int destinationX, int sourceY, int destinationY, int sourceZ, int destinationZ)
{
    Passenger dummy("dummy", 000);
    std::fill_n(std::back_inserter(this->travelList), 1000, dummy);

    this->travelId = travelDate * 100 + sourceX*destinationX + sourceY*destinationX + sourceZ*destinationZ; 
    //unique Travel Id, for each travel, based on source, destination and travelDate
    (this->travelList)[0] = p;
    this->size = 1;
